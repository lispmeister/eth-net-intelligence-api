...
Node.prototype.currentStats = function(){
	return {
		"id": this.id,
		"info" : this.info,
		"stats": this.stats
...
Node.prototype.setupSockets = function()
{
	...
	{
		console.stats('his', 'Got history request: ', data );
	...
	{
		let now = _.now();
		let latency = Math.ceil( (now - data.clientTime) / 2 );
...
Node.prototype.emit = function(message, payload)
{
	if(this._socket)
	{
		try {
			...
			console.sstats('wsc', 'Socket emited message:', chalk.reset.cyan(message), ': ', payload);
...
Node.prototype.getInfo = function()
{
	...
	try {
		this.info.coinbase = this.coinbase();
...
Node.prototype.coinbase = function () {
	//// prevents failure on non-miners with no coinbase set.
  try {
...
Node.prototype.validateLatestBlock = function (error, result, timeString)
{
	...
	let block = this.formatBlock(result);
	...
	{
		let range = _.range( Math.max(this.stats.block.number - MAX_BLOCKS_HISTORY, this._lastBlock + 1), Math.max(this.stats.block.number, 0), 1 );

		console.stats('validateLatestBlock', 'range: ', range);
...
Node.prototype.getHistory = function (range)
{
	...
	console.stats('his', 'range: ', range);
	if ( range === false )
		return // we received faulty data, let's just bail
		
	if ( _.isUndefined(range) || range === null) {
			console.stats('his', '_.isUndefined(range): ', _.isUndefined(range));
			console.stats('his', 'range === null: ', range === null);
			interv = _.range(this.stats.block.number - 1, this.stats.block.number - MAX_HISTORY_UPDATE);
	}
	if (!_.isUndefined(range.list)) {
		console.stats('his','!_.isUndefined(range.list): ', !_.isUndefined(range.list));
		interv = range.list;
	}
	...
	{
		if (err) {
			console.stats('his', 'history fetch failed:', err);
		...
		console.stats('his', 'results: ', results);
...
{
	let changed = ! _.isEqual( this._lastStats, JSON.stringify(this.stats) );
	...
	web3.eth.isSyncing(function(error, sync) {
		if(!error) {
			if(sync === true) {
				...
				let synced = sync.currentBlock - sync.startingBlock;
				let total = sync.highestBlock - sync.startingBlock;
