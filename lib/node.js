...
Node.prototype.getHistory = function (range)
{
  ...
  if (range === false) { // handle a weird edge case
		interv = _.range(this.stats.block.number - 1, this.stats.block.number)
  } else if ( _.isUndefined(range) || range === null) {
			console.error('_.isUndefined(range): ', _.isUndefined(range));
			console.error('range === null: ', range === null);
			interv = _.range(this.stats.block.number - 1, this.stats.block.number - MAX_HISTORY_UPDATE);
	}
	if (!_.isUndefined(range.list)) {
		console.error('!_.isUndefined(range.list): ', !_.isUndefined(range.list));
		interv = range.list;
	}
